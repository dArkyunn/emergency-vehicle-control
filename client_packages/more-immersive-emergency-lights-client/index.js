...
mp.events.add('inputTone6', _ => {
    let entity = mp.players.local.vehicle
    if (entity && entity.type === 'vehicle' && entity.getClass() === 18) {

        if (entity.currentCode >= 3) {
            if (entity.auxSiren == null) {
                entity.auxSiren = false
            }
            if (entity.auxSiren) {
                entity.auxSiren = false
                mp.events.callRemote('syncSiren', entity, false, '', entity.auxSirenID)
            } else {
                entity.auxSiren = true
                if (entity.auxSirenID == null) {
                    let ID = mp.game.invoke('0x430386FE9BF80B45')
                    entity.auxSirenID = ID
                }
                mp.events.callRemote('syncSiren', entity, true, sirenTones.aux, entity.auxSirenID)
            }
        }
    }
})